...
class ApiResovler {
    ...
    upgradeV1ToV2(file) {
        if ((parseInt(file.version) || 0) < 2) {
            ...
            for (const key in file.types) {
                ...
                if (type.native) {
                    type.alias = type.native;
                    delete type.native;
                    if (type.alias == "integer") {
                        type.alias = "number";
                    }
                }
                if (type.fields) {
    ...
    getType(name) {
        ...
        if (!type) {
            ...
            if (/\[\]$/.test(name)) {
                ...
                type.alias = ["string", "number", "boolean", "integer", "date", "void"].indexOf(name) >= 0 ? name : "object";
            }
        }
        return type;
    }
    ...
    mockData(type, value, name, prefix, caseType, depth) {
        if (name) {
            const keyName = name.replace(/^.*_/, "").replace(/^.*([A-Z])/, (all, char) => char.toLowerCase());
